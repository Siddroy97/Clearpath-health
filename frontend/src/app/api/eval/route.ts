...
// ── Tool implementations (mirrors agent/tools.py) ──
...
function execVerifyInsurance(args: { insurance_name: string }): string {
  for (const accepted of ACCEPTED_INSURANCE) {
    if (fuzzyMatch(args.insurance_name, accepted)) {
      return `Yes, ${accepted} is accepted by ClearPath Health. We accept the following plans: ${ACCEPTED_INSURANCE.join(", ")}.`;
    }
  }
...
function execLookupBenefits(args: { member_id: string; benefit_type: string }): string {
  const member = MEMBERS[args.member_id.toUpperCase()] as Record<string, unknown> | undefined;
  if (!member) {
    return `I couldn't locate member ID ${args.member_id}. Could you double-check that number? It should start with MBR followed by digits.`;
  }
  ...
  if (args.benefit_type === "physical_therapy" || args.benefit_type === "mental_health") {
    ...
    if (remaining <= 0) {
      return `${name}, you've used all ${benefit.allowed} of your ${label} visits for this plan year.`;
    }
...
function execCheckDoctorAvailability(args: { specialty?: string; doctor_name?: string }): string {
...
function execBookAppointment(args: { doctor_id: string; slot: string; member_id: string }): string {
  ...
  const conf = `CLR-${Math.floor(100000 + Math.random() * 900000)}`;
  return `Appointment confirmed!\nConfirmation Number: ${conf}\nPatient: ${member.name}\nDoctor: ${doctor.name} (${doctor.specialty})\nTime: ${args.slot}\nLocation: ${doctor.location}`;
}

function execEscalateToHuman(args: { reason: string }): string {
  return `I'm transferring you to a human agent now. I've noted the reason: ${args.reason}. Please stay on the line.`;
}

function execLookupEhrHistory(args: { member_id: string }): string {
...
function executeTool(name: string, args: Record<string, unknown>): string {
  switch (name) {
    case "verify_insurance":
      return execVerifyInsurance(args as { insurance_name: string });
    case "lookup_benefits":
      return execLookupBenefits(args as { member_id: string; benefit_type: string });
    case "check_doctor_availability":
      return execCheckDoctorAvailability(args as { specialty?: string; doctor_name?: string });
    case "book_appointment":
      return execBookAppointment(args as { doctor_id: string; slot: string; member_id: string });
    case "escalate_to_human":
      return execEscalateToHuman(args as { reason: string });
    case "lookup_ehr_history":
      return execLookupEhrHistory(args as { member_id: string });
    default:
...
const TOOL_SCHEMAS: Anthropic.Tool[] = [
  {
    ...
    input_schema: {
      ...
      properties: { insurance_name: { type: "string", description: "The insurance provider name" } },
  ...
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: {
        member_id: { type: "string", description: "The member's ID (e.g. MBR001)" },
        benefit_type: { type: "string", description: "One of: deductible, copay, physical_therapy, mental_health" },
      },
  ...
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: {
        doctor_id: { type: "string", description: "The doctor's ID (e.g. D001)" },
        slot: { type: "string", description: "The time slot (e.g. Monday 9:00 AM)" },
        member_id: { type: "string", description: "The member's ID (e.g. MBR001)" },
      },
  ...
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: { reason: { type: "string", description: "The reason for escalation" } },
      required: ["reason"],
    },
  },
  {
    ...
    input_schema: {
      type: "object" as const,
      properties: { member_id: { type: "string", description: "The member's ID (e.g. MBR001)" } },
      required: ["member_id"],
    },
  },
];

// ── System prompt (mirrors agent/prompts.py) ──
...
export async function POST(req: Request) {
  ...
  for (let turn = 0; turn < 5; turn++) {
    ...
    messages = [...messages, { role: "assistant", content: assistantContent }, toolResults];
  }
